...
void findOffset(Mat blue, Mat green, Mat red, int * offsets) {
	if (blue.rows < 500 && blue.cols < 500) {
		...
		for (int i=-1; i <=1; i++) {
			for (int j=-1; j<=1; j++) {
				printf("accessing green with size (%d, %d) at (%d, %d) with size (%d, %d) \n",
					green.cols, green.rows, wbor+i+2*roffsets[0], hbor+j+2*roffsets[1], blue.cols - 2*wbor, blue.rows - 2*hbor);
